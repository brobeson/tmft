...
def overlap_ratio(rect1, rect2, as_percentage: bool = False):
    """
    ...
    # Why is this clamped? By definition, any IoU should be in [0,1].
    iou = np.clip(intersect / union, 0, 1)
    if as_percentage:
        iou = np.asarray(iou * 100.0, dtype=int)
    return iou


def crop_image2(
    img, bbox, img_size=107, padding=16, flip=False, rotate_limit=0, blur_limit=0
):
    ...
    scaling_matrix = np.asarray(
        [[img_size / w, 0, 0], [0, img_size / h, 0], [0, 0, 1]], dtype=np.float32
    )
    ...
    if rotate_limit and np.random.binomial(1, 0.5):
        ...
        rotation_matrix = np.asarray(
            [[alpha, -beta, 0], [beta, alpha, 0], [0, 0, 1]], dtype=np.float32
        )
        matrices.append(rotation_matrix)

    # Translation matrix to move patch center from origin
    revert_t_matrix = np.asarray(
        [[1, 0, img_size / 2], [0, 1, img_size / 2], [0, 0, 1]], dtype=np.float32
    )
    ...
    patch = cv2.warpPerspective(img, matrix, (img_size, img_size), borderValue=128)
...
def crop_image(img, bbox, img_size=107, padding=16, valid=False):
    ...
    if min_x >= 0 and min_y >= 0 and max_x <= img_w and max_y <= img_h:
        ...
        cropped[
            min_y_val - min_y : max_y_val - min_y,
            min_x_val - min_x : max_x_val - min_x,
            :,
...
# ==================================================================================================
def x(boxes: np.array) -> np.array:
    return boxes[:, 0]


def y(boxes: np.array) -> np.array:
    return boxes[:, 1]


def width(boxes: np.array) -> np.array:
    return boxes[:, 2]


def height(boxes: np.array) -> np.array:
    return boxes[:, 3]


def center_offsets(boxes1, boxes2, whole_pixels=False) -> np.array:
    """
    Calculate the center offsets between two sets of bounding boxes.

    :param np.array boxes1: The first set of bounding boxes to compare. 
    :param np.array boxes2: The second set of bounding boxes to compare. 
    :returns: The list of center offset values.
    :rtype: np.array
    """
    if boxes1.ndim == 1:
        boxes1 = boxes1[None, :]
    ...
    offsets = np.sqrt(
        np.square((x(boxes1) + 0.5 * width(boxes1)) - (x(boxes2) + 0.5 * width(boxes2)))
        + np.square(
            (y(boxes1) + 0.5 * height(boxes1)) - (y(boxes2) + 0.5 * height(boxes2))
        )
    )
    if whole_pixels:
