...
class RegionDataset(data.Dataset):
    def __init__(self, img_list, gt, opts):
        ...
        self.pos_generator = SampleGenerator(
            "uniform", image.size, opts["trans_pos"], opts["scale_pos"]
        )
    ...
    def __next__(self):
        ...
        for i, (img_path, bbox) in enumerate(zip(self.img_list[idx], self.gt[idx])):
            ...
            pos_examples = self.pos_generator(
                bbox, n_pos, overlap_range=self.overlap_pos
            )
            neg_examples = self.neg_generator(
                bbox, n_neg, overlap_range=self.overlap_neg
            )
    ...
    def extract_regions(self, image, samples):
        regions = np.zeros(
            (len(samples), self.crop_size, self.crop_size, 3), dtype="uint8"
        )
        for i, sample in enumerate(samples):
            regions[i] = crop_image2(
                image,
                sample,
                self.crop_size,
                self.padding,
        ...
        regions = regions.astype("float32") - 128.0
        return regions
