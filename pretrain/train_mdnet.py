...
def train_mdnet(opts):
    ...
    criterion = BCELoss()
    evaluator = Precision()
    optimizer = set_optimizer(model, None, opts["lr"], opts["lr_mult"])
    ...
    for i in range(opts["n_cycles"]):
        ...
        for j, k in enumerate(k_list):
            ...
            if j % batch_accum == batch_accum - 1 or j == len(k_list) - 1:
                if "grad_clip" in opts:
                    torch.nn.utils.clip_grad_norm_(
                        model.parameters(), opts["grad_clip"]
                    )
                optimizer.step()
            ...
            toc = time.time() - tic
            print(
                "Cycle {:2d}/{:2d}, Iter {:2d}/{:2d} (Domain {:2d}), Loss {:.3f}, Precision {:.3f}, Time {:.3f}".format(
                    i, opts["n_cycles"], j, len(k_list), k, loss.item(), prec[k], toc
                )
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-d", "--dataset", default="otb", help="training dataset {vot, imagenet, otb}"
    )
