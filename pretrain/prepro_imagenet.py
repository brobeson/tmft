...
seq_home = "/sshd/mamrez/Projects/MDNETOriginal/py-MDNet-master/dataset/ILSVRC/"
...
for num, cur_dir in enumerate(train_list):
    seq_list += [
        os.path.join(cur_dir, p)
        for p in os.listdir(seq_home + "Data/VID/train/" + cur_dir)
    ]
...
for i, seqname in enumerate(seq_list):
    ...
    img_list = sorted(
        [p for p in os.listdir(seq_path) if os.path.splitext(p)[1] == ".JPEG"]
    )
    ...
    gt_list = sorted(
        [
            os.path.join(gt_path, p)
            for p in os.listdir(gt_path)
    ...
    for gidx in range(0, len(img_list)):
        ...
        if (float(doc["annotation"]["size"]["width"]) / 2.0 < xmax - xmin) and (
            float(doc["annotation"]["size"]["height"]) / 2.0 < ymax - ymin
        ):
            continue
